    ...
    <Layout>
      ...
      <section className="py-28 md:py-36 bg-background">
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          <p className="font-mono text-[10px] tracking-widest text-primary mb-4 animate-fade-in">ABOUT</p>
      ...
      {/* Mission */}
      <section ref={missionAnim.ref} className="py-28 bg-card border-y border-border">
        <div className={`max-w-3xl mx-auto px-6 md:px-12 ${missionAnim.isVisible ? "animate-slide-up" : "opacity-0"}`}>
          <h2 className="font-display text-3xl sm:text-4xl font-bold text-foreground mb-8">Our Mission</h2>
          <div className="space-y-6 text-lg text-muted-foreground leading-relaxed">
            <p>Mbote Foundation exists to support bold thinkers, engineers, scientists, and entrepreneurs building solutions for global challenges.</p>
            <p>From artificial intelligence and renewable energy to biotech and space technologies, we believe the next generation of world-changing companies can emerge from Africa.</p>
            <p>We provide funding, mentorship, infrastructure, and a global network to help visionary founders turn breakthrough ideas into reality.</p>
          </div>
        </div>
      </section>

      {/* Locations */}
      <section ref={locAnim.ref} className="py-28 bg-background">
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          <div className={`mb-16 ${locAnim.isVisible ? "animate-slide-up" : "opacity-0"}`}>
            <p className="font-mono text-[10px] tracking-widest text-primary mb-3">LOCATIONS</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {locations.map((loc, i) => (
              <div
                key={loc.name}
                className={`bg-card rounded-xl p-8 border border-border hover:border-primary/25 transition-all duration-300 ${locAnim.isVisible ? "animate-slide-up" : "opacity-0"}`}
                style={{ animationDelay: `${i * 0.08}s` }}
              >
                <div className="flex items-center gap-3 mb-2">
                  <MapPin size={18} className="text-primary" />
                  <span className="text-xs font-mono tracking-widest text-primary">{loc.type}</span>
                </div>
      ...
      {/* Values */}
      <section ref={valAnim.ref} className="py-28 bg-card border-t border-border">
        <div className="max-w-7xl mx-auto px-6 md:px-12">
